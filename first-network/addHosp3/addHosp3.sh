...
function generateHosp3Definition() {
  ...
  echo "##########################################################"
   export FABRIC_CFG_PATH=$PWD
   set -x
   configtxgen -printOrg hosp3MSP > ../organizations/peerOrganizations/hosp3.titanium.com/hosp3.json
   res=$?
   { set +x; } 2>/dev/null
   if [ $res -ne 0 ]; then
...
function addHosp3 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/hosp3.titanium.com" ]; then
    generateHosp3
